...
const Navbar = () => {
  ...
  const { user } = useUser();
  const { openSignIn } = useClerk(); // ✅ Fix: spelling was `OpenSignIn` (should be lowercase)
  ...
  return (
    <div className='fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-36 py-5'>
      ...
      >
        ...
        {favouriteMovie.length > 0 && <Link onClick={() => { scrollTo(0, 0); setIsOpen(false); }} to='/favorite'>Favorites</Link>}
      </div>
      ...
      <div className='flex items-center gap-3 md:gap-8'>
        <SearchIcon className='max-md:hidden w-6 h-6 cursor-pointer' />
        {
          !user ? (
            <button
              onClick={openSignIn} // ✅ Fixed spelling
            ...
            <UserButton>
              <UserButton.MenuItems>
                <UserButton.Action
                  ...
                  onClick={() => navigate('/my-bookings')} // ✅ Fixed: use arrow function
                />
              </UserButton.MenuItems>
            </UserButton>
          )
