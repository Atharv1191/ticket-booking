...
const MovieDetails = () => {
  ...
  const {shows,axios,getToken,user,favouriteMovie, fetchFavouriteMovies,image_base_url} = useAppContext()
  const getShow = async () => {
    try {
  ...
  return show ? (
    <div className='px-6 md:px-16 lg:px-40 pt-30 md:pt-50'>
        ...
        <div className='relative flex flex-col gap-3'>
          ...
          <div className='flex items-center flex-wrap gap-4 mt-4'>
            ...
            <button onClick={handleFavourite} className='bg-gray-700 p-2.5 rounded-full transition cursor-pointer active:scale-95'>
              <Heart className={`w-5 h-5 ${favouriteMovie.find(movie =>movie._id === id)?'fill-primary text-primary':""}`} />
            </button>
          </div>
        </div>
      </div>
      ...
      {/* Cast */}
      <p className='text-xl font-semibold mt-12'>Your Favourite Cast</p>
